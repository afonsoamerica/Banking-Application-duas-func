...
def criar_conta_poupanca(cpf):
    """
    Cria uma conta poupança para o cliente com o CPF fornecido.
    """
    if cpf in poupancas:
        print("O cliente já possui uma conta poupança!")
    else:
        poupancas[cpf] = 0.0  # Inicializa a poupança com saldo zero
        print("Conta poupança criada com sucesso para o CPF:", cpf)

def adicionar_dinheiro_poupanca(cpf, valor):
    """
    Adiciona dinheiro à conta poupança de um cliente.
    """
    if cpf not in poupancas:
        print("Conta poupança não encontrada para o CPF:", cpf)
    elif valor <= 0:
        print("O valor deve ser maior que zero!")
    else:
        poupancas[cpf] += valor
        print(f"R$ {valor:.2f} foram adicionados à poupança. Saldo atual: R$ {poupancas[cpf]:.2f}")

def retirar_dinheiro_poupanca(cpf, valor):
    """
    Retira dinheiro da conta poupança de um cliente.
    """
    if cpf not in poupancas:
        print("Conta poupança não encontrada para o CPF:", cpf)
    elif valor <= 0:
        print("O valor deve ser maior que zero!")
    elif poupancas[cpf] < valor:
        print("Saldo insuficiente na conta poupança!")
    else:
        poupancas[cpf] -= valor
        print(f"R$ {valor:.2f} foram retirados da poupança. Saldo atual: R$ {poupancas[cpf]:.2f}")
...
def menu_interativo():
    ...
    while True:
        ...
        if opcao == 1:
            ...
            adicionar_dinheiro_poupanca(cpf, valor)
        elif opcao == 7:
            cpf = input("Digite o CPF do cliente: ")
            valor = float(input("Digite o valor a retirar da poupança: "))
            retirar_dinheiro_poupanca(cpf, valor)
        elif opcao == 8:
